...
function App(props) {
  ...
  return (
    <div>
      <DivOne updateCapitalization={updateCapitalization} name={state.name} />
    </div>
  );
}
...
function DivOne(props) {
  return (
    <div>
      <h3 onClick={props.updateCapitalization}>{props.name}</h3>
    </div>
  );
}
...
[Thinking in React: The Data Flows Down](https://react.dev/learn/sharing-state-between-components)
...
- After considering the above, take time to read [Thinking in React](https://react.dev/learn/thinking-in-react) again
